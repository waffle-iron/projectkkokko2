...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class TimeCheatingDetector : ActDetectorBase
	{
		...
		private void Update()
		{
			...
			if (asyncResultQueue == null || asyncResultQueue.Count == 0) return;

			lock (lockObject)
			{
				while (asyncResultQueue.Count > 0)
				{
					var callbackData = asyncResultQueue.Dequeue();
					resultList.Add(callbackData);
				}
			}

			if (resultList.Count > 0)
			{
				foreach (var callbackData in resultList)
				{
					callbackData.callback.Invoke(callbackData.data);
				}
		...
		/// <summary>
		/// Allows to manually execute cheating check. Calling it restarts #interval.
		/// </summary>
		public void ForceCheck()
		{
			if (!started || !isRunning)
			{
				Debug.LogWarning(LOG_PREFIX + "Detector should be started to use ForceCheck().");
				return;
			}

			if (IsCheckingForCheat)
			{
				Debug.LogWarning(LOG_PREFIX + "Can't force cheating check since another check is already in progress.");
				return;
			}
		...
		private void SocketAsyncResult(double time)
		{
			if (getOnlineTimeCallback != null)
			{
				lock (lockObject)
				{
					var asyncCallbackData = new AsyncCallbackData();
					asyncCallbackData.callback = getOnlineTimeCallback;
					asyncCallbackData.data = time;

					asyncResultQueue.Enqueue(asyncCallbackData);
				}
			}
		}
