...
public partial class EntitySaveLoader
{
    ...
    public IEntity MakeEntityFromEntityInfo(EntityTemplate EntityTemplate, Contexts contexts)
    {
    ...
    public void AddComponentsFromEntityInfo(IEntity entity, EntityTemplate template)
    {
        AddTagComponents(template, entity);
        AddComponents(template, entity);
    }
    ...
    public EntityTemplate MakeEntityInfo(IEntity entity, string templateName)
    {
        ...
        {
            if (!IsHaveIgnoreSaveAttibute(component) && component.GetType().ToString().Contains("ListenerComponent") == false)
            {
    ...
    private void AddComponents(EntityTemplate EntityTemplate, IEntity newEntity)
    {
        ...
        foreach (KeyValuePair<string, dynamic> componentInfo in EntityTemplate.Components)
        {
            ...
            int componentLookUpIndex = -1;
            Type componentType;
            switch (EntityTemplate.Context)
            {
                case "Game":
                    componentLookUpIndex = (int)typeof(GameComponentsLookup).GetField(componentLookUpName).GetValue(null);
                    componentType = GameComponentsLookup.componentTypes[componentLookUpIndex];
                    break;
                default:
                    throw new Exception($" {EntityTemplate.Context} is not support type. if you have atribute, add it EntitySaveLoaderMakeEntityByContext() ");
            }
            ...
            ((Entity)newEntity).ReplaceComponent(componentLookUpIndex, component as IComponent);
    ...
    private void AddTagComponents(EntityTemplate EntityTemplate, IEntity newEntity)
    {
        if (!string.IsNullOrEmpty(EntityTemplate.Tags))
        {
            ...
            foreach (var tagName in parsedTags)
            {
                ...
                ((Entity)newEntity).ReplaceComponent(componentLookUpIndex, tagComponent as IComponent);
            }
        }
    }
